...
int main() {
    ...
    while (1) {
        ...
        GateOResult res_code;
        res_code = evaluate_command((SignedCommand *)buffer, &sender, &interlocks, now);
        uint32_t result = (uint32_t)res_code;
        write(client_fd, &result, sizeof(result));
        printf("[+] Evaluated command -> Result code: %u\n", result);
        fflush(stdout);
    } else {
